  ...
  <div id="i-form">
    <Form ref="form" :model="model" :rules="rules" :inline="inline" :label-width="labelWidth" :label-position="labelPosition" :style="formWidth" @keyup.enter.native="handleSubmit('form')">
      ...
      <FormItem v-for="(item, index) in elem" :key="index" :prop="item.prop" :label="item.label" :label-width="item.labelWidth" :style="item.width">
        ...
        <!-- 数字输入框 -->
        <Select v-if="item.element === 'select'" v-model="model[item.prop]" :placeholder="item.placeholder" :size="item.size" :disabled="item.disabled" :filterable="item.filterable" :multiple="item.multiple" :style="item.elemWidth" :clearable="item.clearable" :transfer="true">
          <Option v-for=" (opt, index) in item.option" :key="index" :value="opt.value" :disabled="opt.disabled">{{ opt.label }}</Option>
        </Select>
        <!-- 选择器 -->
        <Cascader v-if="item.element === 'cascader'" :data="item.data" v-model="model[item.prop]" :transfer="true" />
        <!-- 级联选择 -->
        <DatePicker v-if="item.element === 'date'" :type="item.type" v-model="model[item.prop]" :options="item.options" :format="item.format" :placeholder="item.placeholder" :size="item.size" :disabled="item.disabled" :style="item.elemWidth" :start-date="item.startdate" :separator="item.separator" :value="item.value" clearable :transfer="true" />
        <!-- 日期选择器 -->
        <TimePicker v-if="item.element === 'time'" :type="item.type" v-model="model[item.prop]" :format="item.format" :placeholder="item.placeholder" :size="item.size" :disabled="item.disabled" :style="item.elemWidth" :transfer="true" />
        <!-- 时间选择器 -->
