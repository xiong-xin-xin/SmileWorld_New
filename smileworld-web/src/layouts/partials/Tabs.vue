<template>
  <div id="tabs">
    <Tabs type="card" :value="$route.path" @on-click="handleTabClick" @on-tab-remove="handleTabRemove">
      <TabPane v-if="menu.length" :key="menu[0]['path']" :name="menu[0]['path']" :label="menu[0]['title']"></TabPane>
      <TabPane v-for="item in tabs" :key="item['path']" :name="item['path']" :label="item['title']" closable></TabPane>
    </Tabs>
    <!-- Tabs -->
