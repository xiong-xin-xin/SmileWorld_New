<template>
  <div id="breadcrumb">
    <router-link v-if="$route.path !== '/'" to="/">
      <Icon v-if="dashboard.icon" :type="dashboard.icon" class="icon" />
      {{ dashboard.title }}
    </router-link>
    <transition-group name="breadcrumb">
      <span v-for="elem in breadcrumb" :key="elem.path">
        <span v-if="$route.path !== '/'" class="separator">/</span>
        <Icon v-if="elem.icon" :type="elem.icon" class="icon" />
        {{ elem.title }}
      </span>
    </transition-group>
  </div>
...
export default {
  ...
  methods: {
    handleBreadcrumb() {
      ...
      this.$store.state.app.curId = curID
    }
  }
};
</script>
